...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uNTC
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        if (fLimitFree && nFees < CTransaction::nMinRelayTxFee)
        {
            ...
            dFreeCount *= pow(1.0 - 1.0/9876.0, (double)(nNow - nLastTime));
            nLastTime = nNow;
...
{
    int64 nSubsidy = 3456 * COIN;

    // Subsidy is cut in half every 4567 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 4567);
...
void ThreadScriptCheck() {
    RenameThread("nothingcoin-scriptch");
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1396302208, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0x298b6777 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0x29, 0x8b, 0x67, 0x77 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NothingcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NothingcoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("NothingcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nothingcoin-miner");
    ...
    try { loop {
        ...
        printf("Running NothingcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NothingcoinMiner terminated\n");
        throw;
    }
}
