...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non NTC unit input in lnReqAmount, we generate the URI with NTC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::NTC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
